...
# Ensure all elements are gseaResult objects
is_gsea_result <- sapply(list_of_gsea, function(x) inherits(x, "gseaResult"))

pval_threshold <- 0.5
sig_threshold <- 0.05

# Function to extract NES scores from gseaResult objects
...
# Apply function to all GSEA objects
...
# Remove rows (pathways) with mostly missing values (optional)
merged_gsea <- merged_gsea[rowSums(!is.na(merged_gsea[,-1])) > 1, ]

# Convert to matrix format for heatmap
nes_matrix <- as.matrix(merged_gsea[,-1])  # Remove pathway column
rownames(nes_matrix) <- merged_gsea$pathway

# Handle missing values (replace NAs with 0 or median imputation)
...
# Reduce font size and adjust clustering
...
y <- DGEList(counts = fitlered_flt_counts_noOutliers)

spike_in_norm <- as.numeric(spike_in_norm)
y$samples$norm.factors <- spike_in_norm


design <- model.matrix(~ logFlowRate  
                       + Lymphocytes 
                       + Total_Monocytes 
                       + Dendritic_Mast 
                       + Eosinophils  
...
contrast.matrix <- makeContrasts(
  ...
  #Ex3 vs Rec2 
  Recovery_vs_Ex3 = TimeRec2.ConditionV - TimeEx3.ConditionV,
...
Baseline_vs_Ex2_LimmaResults <- topTable(fit, coef = "TimeEx2.ConditionV", adjust.method = "BH", number = Inf)
head(Baseline_vs_Ex2_LimmaResults)

Baseline_vs_Rec2_LimmaResults <- topTable(fit, coef = "TimeRec2.ConditionV", adjust.method = "BH", number = Inf)

Rec2_vs_Ex3 <- topTable(fit2, coef = "Recovery_vs_Ex3", adjust.method = "BH", number = Inf)
...
ggplot(Recovery_effect, aes(x = logFC, y = -log(P.Value))) +
...
contrast.matrix <- makeContrasts(
  ...
  Ex2_vs_Baseline = ((TimeEx2.ConditionV - TimeEx2) - 
                       ((X.Intercept.+ ConditionV) - X.Intercept.)),
  
  #Exercise comapred to Recovery
  Recovery_vs_Ex2 = ((TimeRec2.ConditionV - TimeRec2) - 
                       ((X.Intercept.+ ConditionV) - X.Intercept.)),
